...
export async function getHallCalendar(hallId: string, startDate?: string, endDate?: string) {
  let queryText = `
    ...
    WHERE hbc.hallId = $1 AND COALESCE(hbc.isDeleted, FALSE) = FALSE
  `;
  ...
  await query(
    `
      UPDATE HallBookingCalendar SET
        ...
        modifiedAt = CURRENT_TIMESTAMP, modifiedBy = $7
      WHERE id = $8
    `,
...
export async function deleteHallCalendar(id: string, deletedBy?: string) {
  await query("UPDATE HallBookingCalendar SET isDeleted = TRUE, deletedAt = CURRENT_TIMESTAMP, deletedBy = $2 WHERE id = $1", [id, deletedBy || null]);
}
